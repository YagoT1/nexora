...
@section('titulo', 'Nuevo socio')

@section('contenido')
    <h1 class="text-xl font-semibold mb-6">Nuevo socio</h1>

    <form method="POST" action="{{ route('socios.socios.store') }}" class="max-w-md space-y-4 bg-white p-6 border border-gray-200 rounded">
        @csrf

        <div>
            <label class="block text-sm mb-1">Nombre principal</label>
            <input type="text" name="nombre_principal" value="{{ old('nombre_principal') }}" class="w-full border-gray-300 rounded" required>
        ...
        <div>
            ...
            <textarea name="nombres_alternativos" class="w-full border-gray-300 rounded" rows="3">{{ old('nombres_alternativos') }}</textarea>
        ...
        <div>
            ...
            <input type="text" name="dni" value="{{ old('dni') }}" class="w-full border-gray-300 rounded">
        ...
        <div>
            ...
            <input type="text" name="telefono" value="{{ old('telefono') }}" class="w-full border-gray-300 rounded">
            @error('telefono') <p class="text-red-600 text-xs mt-1">{{ $message }}</p> @enderror
        </div>

        <div>
            <label class="block text-sm mb-1">Fecha de alta</label>
            <input type="date" name="fecha_alta" value="{{ old('fecha_alta', now()->toDateString()) }}" class="w-full border-gray-300 rounded" required>
        ...
        <div>
            ...
            <select name="tipo_socio_id" class="w-full border-gray-300 rounded" required>
                ...
                @foreach ($tiposSocio as $tipoSocio)
                    <option value="{{ $tipoSocio->id }}" {{ (int) old('tipo_socio_id') === $tipoSocio->id ? 'selected' : '' }}>
                        {{ $tipoSocio->nombre }}
                    </option>
                @endforeach
            </select>
        ...
        <button type="submit" class="rounded bg-gray-900 text-white text-sm px-4 py-2">Crear socio</button>
    </form>
@endsection
